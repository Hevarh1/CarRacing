...
from rl_baselines_Opt.environment import SubprocVecEnv, make_single_env
from rl_baselines_Opt.models import ContinuousPolicy, DiscretePolicy, MLP, Conv, ValueModel
...
def solve(
    ...
    for epoch in range(epochs):
        ...
        if env.spec.reward_threshold and rets > env.spec.reward_threshold:
            ...
            logger.info(
                f"{env_name}: Check out winning agent `python -m rl_baselines_Opt.test_agent --model={filename} --env={env_name}`"
            )
            return True
